...
int main(int argc, char **argv) {
    ...
    do {
        ...
        while (difftime(endTime, startTime) < 300.0) {
            ...
            while(difftime(endTime, startTime) < 300.0) {
                delay(1);
                if (bcm2835_gpio_lev(level_wash_L) == HIGH) break;
                endTime = time(NULL);
            }
            setOff(pump_wash);
            endTime = time(NULL);
        ...
        //sanitize cycle
        printf("Starting Sanitize Cycle\n");
        setOn(sol_san);
        printf("Starting 1 Min San Loop\n");
        setOn(pump_san);
        sleep(60);
        setOff(pump_san);
        purgeSan();
        ...
        //rinse cycle
        printf("Starting Rinse Cycle (Post-San)\n");
        setOn(sol_recycleRinse);
        rinse();
        purge();
        sleep(10);
        ...
        setOn(sol_drain);
                                                                                            //light & buzzer?
        printf("Complete\n");
        printf("Another Keg to Wash? Y/N: \n");
        scanf("%c", &go);

    } while (go == 'Y' || go == 'y');
    printf("Ending Processes\n");
...
void rinseRecycle(){
    printf("Rinsing w/ Recycled Water...\n");
